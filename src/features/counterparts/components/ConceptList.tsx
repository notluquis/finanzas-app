        ...
        return (
          <div key={concept} className="space-y-1">
            <div className="flex items-center justify-between text-xs text-slate-500">
              ...
              <span className="font-semibold text-(--brand-primary)">{fmtCLP(value)}</span>
            </div>
            <div className="h-2 overflow-hidden rounded-full border border-white/60 bg-base-100/55">
              <div
                className="h-full rounded-full bg-(--brand-primary)/70 shadow-[0_6px_16px_-12px_rgba(16,37,66,0.45)]"
                style={{ width: `${width}%` }}
              />
            </div>
          </div>
