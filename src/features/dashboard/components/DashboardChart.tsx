...
export default function DashboardChart({
  ...
  if (loading) {
    return <div className="p-6 text-sm text-slate-600 bg-base-100">Cargando actividad...</div>;
  }

  if (!data.length) {
    return (
      <div className="p-6 text-sm text-slate-600 bg-base-100">
  ...
  return (
    <article className="space-y-4 p-6 bg-base-100">
      <div className="flex flex-wrap items-center justify-between gap-2">
        ...
        <span className="rounded-full border border-white/60 bg-base-100/60 px-3 py-1 text-[10px] font-semibold uppercase tracking-wide text-slate-500">
          Ingresos vs egresos
        </span>
      </div>
