...
export default function ThemeToggle() {
  ...
  return (
    <div className="dropdown dropdown-end">
      <label tabIndex={0} className="btn btn-ghost btn-circle p-2" aria-haspopup="menu" aria-label={`Tema actual: ${currentLabel}`}>
        {/* simple theme icon */}
      ...
      <div tabIndex={0} className="dropdown-content menu p-3 shadow bg-base-100 rounded-box w-44" role="menu" aria-label="Seleccionar tema">
        <fieldset className="space-y-2">
          <legend className="sr-only">Seleccionar tema</legend>
          <label className="flex items-center gap-3">
            <input type="radio" name="theme" value="light" checked={theme === "light"} onChange={setLight} className="radio radio-sm" />
            <span className="ml-1">Claro</span>
          </label>
          <label className="flex items-center gap-3">
            <input type="radio" name="theme" value="dark" checked={theme === "dark"} onChange={setDark} className="radio radio-sm" />
            <span className="ml-1">Oscuro</span>
          </label>
          <label className="flex items-center gap-3">
            <input type="radio" name="theme" value="system" checked={theme === "system"} onChange={setSystem} className="radio radio-sm" />
